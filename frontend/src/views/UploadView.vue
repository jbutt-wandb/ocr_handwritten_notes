...
const providerLabel = computed(() => PROVIDER_LABELS[configStore.selectedProvider] || configStore.selectedProvider)
    ...
    <div style="max-width: 720px; margin: 0 auto; padding: 36px 40px 48px;">
      ...
      <div v-if="notesStore.images.length > 0" style="margin-bottom: 22px;">
        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 10px;">
          <span style="font-family: 'JetBrains Mono', monospace; font-size: 10px; font-weight: 300; color: #999;">
            {{ notesStore.images.length }}/{{ notesStore.getMaxImages() }} images · drag to reorder
          </span>
          <button
      ...
      <div style="display: flex; flex-direction: column; gap: 8px;">
        <button
          ...
          @click="handleSubmit"
          :disabled="notesStore.isProcessing || !configStore.isSelectedProviderConfigured"
        >
      ...
      <!-- Provider hint -->
      <div style="margin-top: 10px; font-size: 13px; font-style: italic; color: #bbb;">
        <template v-if="configStore.isSelectedProviderConfigured">
          Using {{ providerLabel }} ·
          <button
            type="button"
        ...
        <template v-else>
          Add your {{ providerLabel }} API key via the
          <button
            type="button"
            @click="configStore.openModal('edit')"
            style="background: transparent; border: none; padding: 0; color: #888; cursor: pointer; font-size: 13px; font-style: italic; text-decoration: underline; font-family: 'Crimson Pro', serif;"
