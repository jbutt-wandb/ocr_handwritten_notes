...
async function processAddImages() {
  ...
  try {
    ...
    addError.value = err.message || 'Failed to process images'
  } finally {
    ...
    >
      <div style="background: var(--color-surface); border-radius: 12px; padding: 24px; width: 500px; max-height: 80vh; overflow-y: auto;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
          <h3 style="margin: 0; font-family: 'EB Garamond', Georgia, serif; font-size: 22px; font-weight: 400; color: var(--color-text-primary);">Add More Images</h3>
          <button
            @click="closeAddModal"
        ...
        <div style="margin-bottom: 16px;">
          <div style="display: flex; gap: 16px; margin-bottom: 12px;">
            <label style="display: flex; align-items: center; gap: 6px; cursor: pointer;">
              <input type="checkbox" v-model="addOptions.containsLatex" style="width: 16px; height: 16px; accent-color: var(--color-accent);" />
              <span style="font-size: 14px; color: var(--color-text-primary);">LaTeX equations</span>
            </label>
            <label style="display: flex; align-items: center; gap: 6px; cursor: pointer;">
              <input type="checkbox" v-model="addOptions.containsDiagrams" style="width: 16px; height: 16px; accent-color: var(--color-accent);" />
